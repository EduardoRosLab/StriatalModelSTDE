...
SimetricCosSinState::SimetricCosSinState(unsigned int NumSynapses, float NewMaxMinDistance, float NewCentralAmplitudeFactor, float NewLateralAmplitudeFactor): ConnectionState(NumSynapses, 6), 
...
SimetricCosSinState::~SimetricCosSinState() {
}

float SimetricCosSinState::GetPresynapticActivity(unsigned int index){
	return CentralAmplitudeFactor*this->GetStateVariableAt(index, 0) + LateralAmplitudeFactor*this->GetStateVariableAt(index, 4);
}

float SimetricCosSinState::GetPostsynapticActivity(unsigned int index){
	return 0.0f;
}

unsigned int SimetricCosSinState::GetNumberOfPrintableValues(){
	return ConnectionState::GetNumberOfPrintableValues()+3;
}

double SimetricCosSinState::GetPrintableValuesAt(unsigned int index, unsigned int position){
...
void SimetricCosSinState::SetNewUpdateTime (unsigned int index, double NewTime, bool pre_post){
...
void SimetricCosSinState::ApplyPresynapticSpike(unsigned int index){
	this->incrementStateVariableAt(index, 0, 1.0f);
	this->incrementStateVariableAt(index, 3, 1.0f);
}

void SimetricCosSinState::ApplyPostsynapticSpike(unsigned int index){
	return;
}

void SimetricCosSinState::SetWeight(unsigned int index, float weight, float max_weight){
}
