...
ModelDescription ExpBufferedWeightChange::ParseLearningRule(FILE * fh) throw (EDLUTException) {
...
void ExpBufferedWeightChange::SetParameters(std::map<std::string, boost::any> param_map) throw (EDLUTException){
...
std::map<std::string,boost::any> ExpBufferedWeightChange::GetDefaultParameters(){
	...
	newMap["init_time"] = boost::any(0.0f);
	return newMap;
}
