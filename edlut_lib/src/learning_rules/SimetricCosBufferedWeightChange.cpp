...
SimetricCosBufferedWeightChange::SimetricCosBufferedWeightChange() :WithoutPostSynaptic(),
...
void SimetricCosBufferedWeightChange::InitializeConnectionState(unsigned int NumberOfSynapses, unsigned int NumberOfNeurons){
	...
	while (1){
		...
		if (exp(-maxTimeMeasured*exponent)*sin(1.5708f*maxTimeMeasured)*sin(1.5708f*maxTimeMeasured)< tolerance){
			break;
		}
	}
...
ModelDescription SimetricCosBufferedWeightChange::ParseLearningRule(FILE * fh) throw (EDLUTException) {
...
void SimetricCosBufferedWeightChange::SetParameters(std::map<std::string, boost::any> param_map) throw (EDLUTException){
