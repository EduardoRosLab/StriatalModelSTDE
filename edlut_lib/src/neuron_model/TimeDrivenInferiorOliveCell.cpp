	...
	if (I_COU){
		for (int i = 0; i < this->GetVectorNeuronState()->GetSizeState(); i++){
			...
			for (int j = 0; j < this->N_coupling_synapses[i]; j++){
				...
				float V = NeuronState[V_m_index] - source_V;
...
std::map<std::string, boost::any> TimeDrivenInferiorOliveCell::GetSpecificNeuronParameters(int index) const throw (EDLUTException){
	return GetParameters();
}

void TimeDrivenInferiorOliveCell::SetParameters(std::map<std::string, boost::any> param_map) throw (EDLUTException){
...
IntegrationMethod * TimeDrivenInferiorOliveCell::CreateIntegrationMethod(ModelDescription imethodDescription) throw (EDLUTException){
...
ModelDescription TimeDrivenInferiorOliveCell::ParseNeuronModel(std::string FileName) throw (EDLUTFileException){
	FILE *fh;
