			...
			if(fscanf(fh,"%i",&FiringIndex)==1){
				...
				if(fscanf(fh,"%i",&this->NumSynapticVar)==1){
					...
					if(fscanf(fh,"%i",&this->LastSpikeVar)!=1){
						throw EDLUTFileException(TASK_TABLE_BASED_MODEL_LOAD, ERROR_TABLE_BASED_MODEL_LAST_SPIKE, REPAIR_NEURON_MODEL_TABLE_TABLES_STRUCTURE, Currentline, ConfigFile.c_str());
					}

					skip_comments(fh,Currentline);
					if(fscanf(fh,"%i",&this->SeedVar)!=1){
						throw EDLUTFileException(TASK_TABLE_BASED_MODEL_LOAD, ERROR_TABLE_BASED_MODEL_SEED, REPAIR_NEURON_MODEL_TABLE_TABLES_STRUCTURE, Currentline, ConfigFile.c_str());
					}
