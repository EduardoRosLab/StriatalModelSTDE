...
std::map<std::string, boost::any> SinCurrentDeviceVector::GetSpecificNeuronParameters(int index) const throw (EDLUTException){
...
void SinCurrentDeviceVector::SetParameters(std::map<std::string, boost::any> param_map) throw (EDLUTException){
...
void SinCurrentDeviceVector::SetSpecificNeuronParameters(int index, std::map<std::string, boost::any> param_map) throw (EDLUTException){
	//This method must be executed after the initilization of the model.
	if (this->State->GetSizeState() == 0){
		//The neuron model has not been initialized
		throw EDLUTException(TASK_GET_NEURON_SPECIFIC_PARAMETERS, ERROR_NON_INITIALIZED_SIMULATION, REPAIR_EXECUTE_AFTER_INITIALIZE_SIMULATION);
...
ModelDescription SinCurrentDeviceVector::ParseNeuronModel(std::string FileName) throw (EDLUTFileException){
	FILE *fh;
