...
const ERROR_STRUCT EDLUTException::ErrorMsgs[] = {
	...
	{ ERROR_DOPAMINE_STDP_WEIGHT_CHANGE_TAUS, "LTP tau (second parameter), LTD tau (fourth parameter), channel tau (fifth parameter), dopamine tau (sixth parameter) must be greater than zero" },
	...
	{ ERROR_LIF_TIME_DRIVEN_MODEL_MAX_DA, "Can't read the maximum dopamine value (thirteenth parameter). This parameter must be set in (REVISAR) units" },
	{ ERROR_LIF_TIME_DRIVEN_MODEL_DA_THR_FAC, "Can't read the dopamine influence on threshold factor (twelfth parameter). This parameter must be set in dimensionless units" },
	{ ERROR_LIF_TIME_DRIVEN_MODEL_TAU_DA, "Can't read the DA receptor time constant (eleventh parameter). This parameter must be set in ms units" },
