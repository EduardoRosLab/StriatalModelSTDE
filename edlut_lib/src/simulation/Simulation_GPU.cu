...
Simulation::Simulation(const std::list<NeuronLayerDescription> & neuron_layer_list,
					   ...
					   int NewNumberOfQueues) throw (EDLUTException) :
	...
	for(int i=0; i<this->NumberOfQueues; i++){
		...
		for(int z=0; z<this->GetNetwork()->GetNneutypes(); z++){
			if(N_TimeDrivenNeuronGPU[z][i]>0){
				...
				this->Queue->InsertEvent(i, new TimeEventUpdateNeurons(model->GetTimeDrivenStepSize(), i, model, this->GetNetwork()->GetTimeDrivenNeuronGPUAt(z, i)));
			}
		}
