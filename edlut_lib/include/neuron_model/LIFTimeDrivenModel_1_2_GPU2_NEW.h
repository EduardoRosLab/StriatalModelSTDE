/***************************************************************************
 *                           LIFTimeDrivenModel_1_2_GPU2_NEW.h             *
 *                           -------------------                           *
...
#ifndef LIFTIMEDRIVENMODEL_1_2_GPU2_NEW_H_
#define LIFTIMEDRIVENMODEL_1_2_GPU2_NEW_H_

/*!
 * \file LIFTimeDrivenModel_1_2_GPU2_NEW.h
 *
 ...
 * implemented in GPU.This class is a redefinition of LIFTimeDrivenModel_1_2_GPU2 neuron 
...
class LIFTimeDrivenModel_1_2_GPU2_NEW : public TimeDrivenNeuronModel_GPU2 {
	public:
		/*!
		...
		*/
		 ...
		 *
		 */
		__device__ LIFTimeDrivenModel_1_2_GPU2_NEW(float Eexc, float Einh, float Erest, float Vthr, float Cm, float Texc, float Tinh,
				float Tref, float Grest, char const* integrationName, int N_neurons, void ** Buffer_GPU):TimeDrivenNeuronModel_GPU2(MilisecondScale_GPU),
		 ...
		 */
		__device__ virtual ~LIFTimeDrivenModel_1_2_GPU2_NEW(){
		...
		__device__ void UpdateState(double CurrentTime)
		{
			...
			while (index<vectorNeuronState_GPU2->SizeStates){
				...
				vectorNeuronState_GPU2->VectorNeuronStates_GPU[1*vectorNeuronState_GPU2->SizeStates + index]+=vectorNeuronState_GPU2->AuxStateGPU[index];                                        //gexc
				vectorNeuronState_GPU2->VectorNeuronStates_GPU[2*vectorNeuronState_GPU2->SizeStates + index]+=vectorNeuronState_GPU2->AuxStateGPU[vectorNeuronState_GPU2->SizeStates + index];   //ginh
		...
		__device__ void EvaluateDifferentialEcuation(int index, int SizeStates, float * NeuronState, float * AuxNeuronState, float elapsed_time){
			if(vectorNeuronState_GPU2->LastSpikeTimeGPU[index] * this->GetTimeScale() > this->tref){
				...
				AuxNeuronState[blockDim.x*blockIdx.x + threadIdx.x]=fraction*(NeuronState[SizeStates + index] * (this->eexc - NeuronState[index]) + NeuronState[2*SizeStates + index] * (this->einh - NeuronState[index]) + grest * (this->erest - NeuronState[index]))*this->inv_cm;
			}else{
...
#endif /* LIFTIMEDRIVENMODEL_1_2_GPU2_NEW_H_ */
