/***************************************************************************
 *                           LIFTimeDrivenModel_1_4_GPU2.cuh               *
 *                           -------------------                           *
...
class LIFTimeDrivenModel_1_4_GPU2 : public TimeDrivenNeuronModel_GPU2 {
	public:
		...
		*/
		 ...
		 */
		const float tau_gap_jun;
		const float inv_tau_gap_jun;

		/*!
		 * \brief Gap junction factor in V/S units
		 */
		const float gap_jun_fac;
		const float gap_jun_fac_VnS; //Auxiliar Gap junction factor in V/nS units
		...
		*/
		 ...
		 * \param g_leak g_leak.
		 * \param tau_nmda tau_nmda.
		 * \param tau_gap_jun tau_gap_jun.
		 * \param gap_jun_fac gap_jun_fac.
		 * \param integrationName integration method type.
		 * \param N_neurons number of neurons.
		 * \param Buffer_GPU Gpu auxiliar memory.
		 *
		 */
		__device__ LIFTimeDrivenModel_1_4_GPU2(float e_exc, float e_inh, float e_leak, float v_thr, float c_m, float tau_exc, float tau_inh,
				float tau_ref, float g_leak, float tau_nmda, float tau_gap_jun, float gap_jun_fac, char const* integrationName, int N_neurons, void ** Buffer_GPU):TimeDrivenNeuronModel_GPU2(SecondScale_GPU),
				...
				tau_inh(tau_inh), inv_tau_inh(1.0f/tau_inh), tau_ref(tau_ref), g_leak(g_leak), g_leak_nS(g_leak*1.e9), tau_nmda(tau_nmda), inv_tau_nmda(1.0/tau_nmda),
				tau_gap_jun(tau_gap_jun), inv_tau_gap_jun(1.0/tau_gap_jun), gap_jun_fac(gap_jun_fac), gap_jun_fac_VnS(gap_jun_fac*1.e-9){

			this->integration_method_GPU2 = IntegrationMethodFactory_GPU2<LIFTimeDrivenModel_1_4_GPU2>::loadIntegrationMethod_GPU2(integrationName, Buffer_GPU, this);
		...
		__device__ void EvaluateSpikeCondition(float previous_V, float * NeuronState, int index, float elapsedTimeInNeuronModelScale){
			float vm_cou = NeuronState[index] + this->gap_jun_fac * NeuronState[4*vectorNeuronState_GPU2->SizeStates + index];
			if (vm_cou > this->v_thr){		
		...
		__device__ void EvaluateDifferentialEquation(int index, int SizeStates, float * NeuronState, float * AuxNeuronState, float elapsed_time){
			if(vectorNeuronState_GPU2->LastSpikeTimeGPU[index] * this->GetTimeScale() > this->tau_ref){
				float i_exc = NeuronState[SizeStates + index]*(this->e_exc-NeuronState[index]);
				float i_inh = NeuronState[3*SizeStates + index]*(this->e_inh-NeuronState[index]);
				float g_nmda_inf = 1.0f/(1.0f + expf(-62.0f*NeuronState[index])*(1.2f/3.57f));
				float i_nmda = NeuronState[2*SizeStates + index]*g_nmda_inf*(this->e_exc-NeuronState[index]);
				AuxNeuronState[blockDim.x*blockIdx.x + threadIdx.x]=(i_exc + i_inh + i_nmda  + this->g_leak_nS * (this->e_leak-NeuronState[index]))*this->inv_c_m_nF;
			}else if ((vectorNeuronState_GPU2->LastSpikeTimeGPU[index] * this->GetTimeScale() + elapsed_time)>this->tau_ref){
				float fraction = (this->vectorNeuronState_GPU2->LastSpikeTimeGPU[index] * this->GetTimeScale() + elapsed_time - this->tau_ref) / elapsed_time;
				float i_exc = NeuronState[SizeStates + index]*(this->e_exc-NeuronState[index]);
				float i_inh = NeuronState[3*SizeStates + index]*(this->e_inh-NeuronState[index]);
				float g_nmda_inf = 1.0f/(1.0f + expf(-62.0f*NeuronState[index])*(1.2f/3.57f));
				float i_nmda = NeuronState[2*SizeStates + index]*g_nmda_inf*(this->e_exc-NeuronState[index]);
				AuxNeuronState[blockDim.x*blockIdx.x + threadIdx.x]=fraction*((i_exc + i_inh + i_nmda  + this->g_leak_nS * (this->e_leak-NeuronState[index]))*this->inv_c_m_nF);
			}else{
		...
		__device__ void Calculate_conductance_exp_values(int index, float elapsed_time){
			...
			Set_conductance_exp_values(index, 3, expf(-elapsed_time*this->inv_tau_gap_jun));
