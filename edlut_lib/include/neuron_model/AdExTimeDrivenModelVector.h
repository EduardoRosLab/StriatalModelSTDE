...
#ifndef ADEXTIMEDRIVENMODELVECTOR_H_
#define ADEXTIMEDRIVENMODELVECTOR_H_

/*!
 * \file AdExTimeDrivenModelVector.h
 *
 * \author Francisco Naveros
 * \date April 2020
 *
 * This file declares a class which implements an Adaptative Exponential Integrate and Fire (AdEx) neuron model with two 
 * differential equations (membrane potential and membrane recovery), three time dependent equations (excitatory, inhibitory and
...
/*!
 * \class AdExTimeDrivenModelVector
 *
 * \brief AdEx Time-Driven neuron model with a membrane potential (V), a membrane recovery variable (w), three
 * conductances (excitatory, inhibitory and NMDA conductances) and one external input current synapse. This is
 * a vectorized model in which each neuron can take different values in several configuration parameters.
 *
 ...
 * \date April 2020
 */
class AdExTimeDrivenModelVector : public TimeDrivenNeuronModel {
	protected:
		struct VectorParameters{
			float a;
			float b;
			float thr_slo_fac;
			float inv_thr_slo_fac;
			float v_thr;
			//float tau_w;
			float inv_tau_w;
			//float e_exc;
		...
		*
		* It computest the g_nmda_inf values based on the fixed e_exc and e_inh values.
		*/
	...
	public:
		...
		*/
		 ...
		 */
		virtual ~AdExTimeDrivenModelVector();
		...
		* NOTE: this function is accesible throgh the Simulatiob_API interface.
		*/
		virtual void SetSpecificNeuronParameters(int index, std::map<std::string, boost::any> param_map) throw (EDLUTException);
		...
		*/
        ...
        virtual bool compare(const NeuronModel * rhs) const{
            ...
            const AdExTimeDrivenModelVector * e = dynamic_cast<const AdExTimeDrivenModelVector *> (rhs);
            if (e == 0) return false;

            return this->a==e->a &&
                   this->b==e->b &&
                   this->thr_slo_fac==e->thr_slo_fac &&
                   this->v_thr==e->v_thr &&
                   this->tau_w==e->tau_w &&
                   this->e_exc==e->e_exc &&
                   this->e_inh==e->e_inh &&
				   this->e_reset == e->e_reset &&
...
#endif /* ADEXTIMEDRIVENMODELVECTOR_H_ */
