...
#ifndef SINCURRENTDEVICEVECTOR_H_
#define SINCURRENTDEVICEVECTOR_H_

/*!
 * \file SinCurrentDeviceVector.h
 *
 * \author Francisco Naveros
 * \date April 2020
 *
 * This file declares a class which implements a current generator device with sinusoidal shapes. This model 
 * does not support input synapses.
 *
...
/*!
 * \class SinCurrentDeviceVector
 *
 * \brief Current generator device with sinosoidal shapes.This is a vectorized model in which each 
 * neuron can take different values in several configuration parameters.
 *
 *
 * \author Francisco Naveros
 * \date April 2020
 */
class SinCurrentDeviceVector : public TimeDrivenInputDevice {
	protected:
		...
		*/
		...
		/*!
		 * \brief Sinusoidal amplitude in pA units
		 */
		float amplitude;

		/*!
		 * \brief Sinusoidal offset in pA units
		 */
		float offset;

		/*!
		 * \brief Sinusoidal phase in rad units
		 */
		float phase;
	...
	public:
		 ...
		 */
		virtual ~SinCurrentDeviceVector();
		...
		*/
        ...
        virtual bool compare(const NeuronModel * rhs) const{
            ...
            const SinCurrentDeviceVector * e = dynamic_cast<const SinCurrentDeviceVector *> (rhs);
            if (e == 0) return false;
...
#endif /* SINCURRENTDEVICEVECTOR_H_ */
