 ...
 * \date November 2019
 *
 * This file declares a class which implements a Purkinje cell model with three differential equations
 * (membrane potential, calcium and Muscariny channels), three time dependent equations (excitatory, inhibitory and NMDA conductances)
 ...
 *
 * \brief Time driven neuron model representing a Purkinge cell with a membrane potential, two calcium and Muscariny channels,
 * three time dependent equations (excitatory, inhibitory and NMDA conductances) and one external input current synapse.
 *
 * \author Richard Carrillo
 * \author Niceto Luque
 * \author Francisco Naveros
	...
	public:
		...
		*/
        ...
        virtual bool compare(const NeuronModel * rhs) const{
			...
			const TimeDrivenPurkinjeCell * e = dynamic_cast<const TimeDrivenPurkinjeCell *> (rhs);
            if (e == 0) return false;
