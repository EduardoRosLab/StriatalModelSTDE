...
#ifndef LIFTIMEDRIVENMODEL_IS_H_
#define LIFTIMEDRIVENMODEL_IS_H_

/*!
 * \file LIFTimeDrivenModel_IS.h
 *
 ...
 * NMDA conductances) and one external input current synapse. In this case, the model uses the units
 * defined in the International System (IS): seconds (s), volts (V), faradiums (F) and siemens (S) instead
 * of miliseconds (ms), milivolts (mV), picofaradiums (pF) and nanosiemens (nS).
 */
 ...
 * \date November 2019
 */
class LIFTimeDrivenModel_IS : public TimeDrivenNeuronModel {
	protected:
	...
	public:
		...
		*/
        ...
        virtual bool compare(const NeuronModel * rhs) const{
            ...
            const LIFTimeDrivenModel_IS * e = dynamic_cast<const LIFTimeDrivenModel_IS *> (rhs);
            if (e == 0) return false;
...
#endif /* LIFTIMEDRIVENMODEL_IS_H_ */
