 ...
 * \date November 2019
 *
 * This file declares a class which implements a Hodgkin and Huxley neuron model with four differential equations
 * (membrane potential, m, h and n), three time dependent equations (excitatory, inhibitory and NMDA conductances)
 * and one external input current synapse.
 */
...
/*!
 * \class HHTimeDrivenModel
 *
 * \brief Hodgkin and Huxley Time-Driven neuron model with a membrane potential, three ion-channel variables (m, h and n), three
...
class HHTimeDrivenModel : public TimeDrivenNeuronModel {
	protected:
		...
		*/
		static const int TableSize=1024*256;
	...
	public:
		...
		*/
		...
		void Generate_channel_values(){
			...
			for(int i=0; i<TableSize; i++){
				...
				//alpha_m
				float alpha_m;
				if((13.0f-V+v_thr)==0.0){
					alpha_m=1.279999894092334f;
				}else{
					alpha_m=0.32f*(13.0f-V+v_thr)/(exp((13.0f-V+v_thr)*0.25f)-1.0f);
				}
				channel_values[i*N_variables] = alpha_m;

				//beta_m
				float beta_m;
				if((V-v_thr-40.0f)==0.0){
					beta_m=1.400030971093304f;
				}else{
					beta_m=0.28f*(V-v_thr-40.0f)/(exp((V-v_thr-40.0f)*0.2f)-1.0f);
				}
				channel_values[i*N_variables + 1] = beta_m;

				//alpha_h
				float alpha_h=0.128f*exp((17.0f-V+v_thr)*0.05555555f);
				channel_values[i*N_variables + 2] = alpha_h;

				//beta_h
				float beta_h=4.0f/(1.0f+exp((40.0f-V+v_thr)*0.2f));
				channel_values[i*N_variables + 3] = beta_h;

				//alpha_n
				float alpha_n;
				if((15.0f-V+v_thr)==0.0){
					alpha_n=0.1599999867615418f;
				}else{
					alpha_n=0.032f*(15.0f-V+v_thr)/(exp((15.0f-V+v_thr)*0.2f)-1.0f);
				}
				channel_values[i*N_variables + 4] = alpha_n;

				//beta_n
				float beta_n=0.5f*exp((10.0f-V+v_thr)*0.025f);
		...
		*/
        ...
        virtual bool compare(const NeuronModel * rhs) const{
            ...
            const HHTimeDrivenModel * e = dynamic_cast<const HHTimeDrivenModel *> (rhs);
            if (e == 0) return false;
