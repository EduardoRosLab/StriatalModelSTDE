...
#ifndef LIFTIMEDRIVENMODEL_DA_H_
#define LIFTIMEDRIVENMODEL_DA_H_
 ...
 * \date November 2019
 *
 * This file declares a class which implements a Leaky Integrate-And-Fire (LIF) neuron model with one
 ...
 * \date November 2019
 */
class LIFTimeDrivenModel_DA : public TimeDrivenNeuronModel {
	protected:
		...
		*/
		...
		/*!
		 * \brief Dopamine receptor time constant in ms units
		 */
		float tau_da;
		float inv_tau_da;

		/*!
		 * \brief Dopamine influence on threshold factor.
		 */
		float da_thr_fac;

		/*!
		 * \brief Max dopamine value.
		 */
		float max_da;
	...
	public:
		 ...
		 */
		const int N_TimeDependentNeuronState = 7;
			...
			const int DA_index = N_DifferentialNeuronState + 4;
			const int TIME_index = N_DifferentialNeuronState + 5; //This is not a synapse type, but a neuron state for dynamic threshold
			const int DYN_THR_index = N_DifferentialNeuronState + 6; //This is not a synapse type, but a neuron state for dynamic threshold
		...
		*/
        ...
        virtual bool compare(const NeuronModel * rhs) const{
            ...
            const LIFTimeDrivenModel_DA * e = dynamic_cast<const LIFTimeDrivenModel_DA *> (rhs);
            if (e == 0) return false;
            ...
            return this->e_exc==e->e_exc &&
                   ...
                   this->tau_ref==e->tau_ref &&
				   		 	   ...
				   		 	   this->tau_da==e->tau_da &&
									 this->da_thr_fac==e->da_thr_fac &&
									 this->max_da==e->max_da &&
...
#endif /* LIFTIMEDRIVENMODEL_DA_H_ */
