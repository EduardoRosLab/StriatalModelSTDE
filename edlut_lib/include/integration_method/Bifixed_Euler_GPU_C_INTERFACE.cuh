/***************************************************************************
 *                           Bifixed_Euler_GPU_C_INTERFACE.cuh             *
 *                           -------------------                           *
...
#ifndef BIFIXED_EULER_GPU_C_INTERFACE_H_
#define BIFIXED_EULER_GPU_C_INTERFACE_H_

/*!
 * \file Bifixed_Euler_GPU_C_INTERFACE.cuh
 *
...
/*!
 * \class Bifixed_Euler_GPU_C_INTERFACE
 *
...
class Bifixed_Euler_GPU_C_INTERFACE: public BifixedStep_GPU_C_INTERFACE<Neuron_Model_GPU>{
	public:
     ...
     */
		Bifixed_Euler_GPU_C_INTERFACE():BifixedStep_GPU_C_INTERFACE<Neuron_Model_GPU>(), AuxNeuronState(0){
		}
		 ...
		 *
		 * It generates a new Bifixed Euler object.
		 *
		* \param NewModel pointer to a template class representing the neuron model.
		*/

		Bifixed_Euler_GPU_C_INTERFACE(Neuron_Model_GPU *  NewModel) :BifixedStep_GPU_C_INTERFACE<Neuron_Model_GPU>(NewModel), AuxNeuronState(0){
		}
		 ...
		 */
		virtual ~Bifixed_Euler_GPU_C_INTERFACE(){
			if (AuxNeuronState != 0){
		...
		void InitializeMemoryGPU(int N_neurons, int Total_N_thread)
		{
			...
			Call_Euler_GPU_C_INTERFACE_memory(this->Buffer_GPU, integration_method_parameters_GPU, AuxNeuronState);
		...
		std::ostream & PrintInfo(std::ostream & out){
			out << "\t\tIntegration Method Type: " << Bifixed_Euler_GPU_C_INTERFACE::GetName() << endl;
		...
		virtual std::map<std::string, boost::any> GetParameters() const{
			...
			newMap["name"] = Bifixed_Euler_GPU_C_INTERFACE::GetName();
			return newMap;
		}
		...
		static ModelDescription ParseIntegrationMethod(FILE * fh, long & Currentline) throw (EDLUTException){
			...
			nmodel.model_name = Bifixed_Euler_GPU_C_INTERFACE::GetName();
			return nmodel;
		}
		...
		*/
		static IntegrationMethod_GPU_C_INTERFACE* CreateIntegrationMethod(ModelDescription nmDescription, Neuron_Model_GPU *nmodel){
			Bifixed_Euler_GPU_C_INTERFACE * newmodel = new Bifixed_Euler_GPU_C_INTERFACE(nmodel);
			...
			const Bifixed_Euler_GPU_C_INTERFACE *e = dynamic_cast<const Bifixed_Euler_GPU_C_INTERFACE *> (rhs);
			if (e == 0) return false;
			return true;
	        };
...
#endif /* BIFIXED_EULER_GPU_C_INTERFACE_H_ */
