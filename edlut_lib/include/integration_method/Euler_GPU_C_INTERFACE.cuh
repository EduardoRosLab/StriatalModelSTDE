...
#ifndef EULER_GPU_C_INTERFACE_H_
#define EULER_GPU_C_INTERFACE_H_

/*!
 * \file Euler_GPU_C_INTERFACE.cuh
 *
...
class Euler_GPU_C_INTERFACE : public FixedStep_GPU_C_INTERFACE<Neuron_Model_GPU>{
	public:
     ...
     */
		Euler_GPU_C_INTERFACE():FixedStep_GPU_C_INTERFACE<Neuron_Model_GPU>(), AuxNeuronState(0){
		}
		...
		*/
		...
		Euler_GPU_C_INTERFACE(Neuron_Model_GPU *  NewModel):FixedStep_GPU_C_INTERFACE<Neuron_Model_GPU>(NewModel), AuxNeuronState(0){
		}
		...
		std::ostream & PrintInfo(std::ostream & out){
			out << "\t\tIntegration Method Type: " << Euler_GPU_C_INTERFACE::GetName() << endl;
		...
		virtual std::map<std::string, boost::any> GetParameters() const{
			...
			newMap["name"] = Euler_GPU_C_INTERFACE::GetName();
			return newMap;
		}
		...
		static ModelDescription ParseIntegrationMethod(FILE * fh, long & Currentline) throw (EDLUTException){
			...
			nmodel.model_name = Euler_GPU_C_INTERFACE::GetName();
			return nmodel;
		}
		...
		*/
		static IntegrationMethod_GPU_C_INTERFACE* CreateIntegrationMethod(ModelDescription nmDescription, Neuron_Model_GPU *nmodel){
			Euler_GPU_C_INTERFACE * newmodel = new Euler_GPU_C_INTERFACE(nmodel);
			...
			const Euler_GPU_C_INTERFACE *e = dynamic_cast<const Euler_GPU_C_INTERFACE *> (rhs);
			if (e == 0) return false;
			return true;
	        };
...
#endif /* EULER_GPU_C_INTERFACE_H_ */
