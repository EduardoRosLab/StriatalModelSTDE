...
#ifndef RK2_GPU_C_INTERFACE_H_
#define RK2_GPU_C_INTERFACE_H_

/*!
 * \file RK2_GPU_C_INTERFACE.cuh
 *
...
class RK2_GPU_C_INTERFACE : public FixedStep_GPU_C_INTERFACE<Neuron_Model_GPU>{
	public:
		...
		*/
		...
		/*!
     		...
     		*/
		RK2_GPU_C_INTERFACE():FixedStep_GPU_C_INTERFACE<Neuron_Model_GPU>(), AuxNeuronState1(0), AuxNeuronState2(0){
		}
		...
		*/
		...
		RK2_GPU_C_INTERFACE(Neuron_Model_GPU *  NewModel) :FixedStep_GPU_C_INTERFACE<Neuron_Model_GPU>(NewModel), AuxNeuronState1(0), AuxNeuronState2(0){
		}
		...
		std::ostream & PrintInfo(std::ostream & out){
			out << "\t\tIntegration Method Type: " << RK2_GPU_C_INTERFACE::GetName() << endl;
		...
		virtual std::map<std::string, boost::any> GetParameters() const{
			...
			newMap["name"] = RK2_GPU_C_INTERFACE::GetName();
			return newMap;
		}
		...
		static ModelDescription ParseIntegrationMethod(FILE * fh, long & Currentline) throw (EDLUTException){
			...
			nmodel.model_name = RK2_GPU_C_INTERFACE::GetName();
			return nmodel;
		}
		...
		*/
		static IntegrationMethod_GPU_C_INTERFACE* CreateIntegrationMethod(ModelDescription nmDescription, Neuron_Model_GPU *nmodel){
			RK2_GPU_C_INTERFACE * newmodel = new RK2_GPU_C_INTERFACE(nmodel);
			...
			const RK2_GPU_C_INTERFACE *e = dynamic_cast<const RK2_GPU_C_INTERFACE *> (rhs);
			if (e == 0) return false;
			return true;
        	};
