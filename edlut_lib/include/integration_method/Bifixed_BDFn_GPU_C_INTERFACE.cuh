/***************************************************************************
 *                           Bifixed_BDFn_GPU_C_INTERFACE.cuh              *
 *                           -------------------                           *
...
#ifndef BIFIXED_BDFN_GPU_C_INTERFACE_H_
#define BIFIXED_BDFN_GPU_C_INTERFACE_H_

/*!
 * \file Bifixed_BDFn_GPU_C_INTERFACE.cuh
 *
 * \author Francisco Naveros
 * \date April 2020
 *
 * This file declares a class which implements two multi step BDF (Backward Differentiation Formulas) integration methods (from
 * first and second order BDF integration method) in GPU (this class is stored in CPU memory and controles the
...
#include "integration_method/BifixedStep_GPU_C_INTERFACE.cuh"
...
/*!
 * \class Bifixed_BDFn_GPU_C_INTERFACE
 *
 * \brief BDFn integration method in CPU for GPU
 *
 * This class abstracts the initializacion in CPU of two BDF integration methods (BDF1 and BDF2) for GPU.
...
class Bifixed_BDFn_GPU_C_INTERFACE : public BifixedStep_GPU_C_INTERFACE<Neuron_Model_GPU>{
	public:
		...
		*/
		...
		/*!
     		...
     		*/
		Bifixed_BDFn_GPU_C_INTERFACE():BifixedStep_GPU_C_INTERFACE<Neuron_Model_GPU>(), AuxNeuronState(0), AuxNeuronState_p(0),
		...
		*/
		...
		Bifixed_BDFn_GPU_C_INTERFACE(Neuron_Model_GPU *  NewModel):BifixedStep_GPU_C_INTERFACE<Neuron_Model_GPU>(NewModel), AuxNeuronState(0), AuxNeuronState_p(0),
			AuxNeuronState_p1(0), AuxNeuronState_c(0), jacnum(0), J(0), inv_J(0), AuxNeuronState2(0), AuxNeuronState_pos(0),
			AuxNeuronState_neg(0), Coeficient(0), PreviousNeuronState(0), D(0), state(0){
		}
		 ...
		 */
		virtual ~Bifixed_BDFn_GPU_C_INTERFACE(){
			if (AuxNeuronState != 0){
		...
		void InitializeMemoryGPU(int N_neurons, int Total_N_thread)
		{
			...
			float integration_method_parameters_CPU[4];
			...
			integration_method_parameters_CPU[3] = this->BDForder;
			float * integration_method_parameters_GPU;
			cudaMalloc((void**)&integration_method_parameters_GPU, 4 * sizeof(float));
			cudaMemcpy(integration_method_parameters_GPU, integration_method_parameters_CPU, 4 * sizeof(float), cudaMemcpyHostToDevice);
			...
			cudaMalloc((void**)&Coeficient, 3*4*sizeof(float));
			float Coeficient_CPU[3 * 4] = { 1.0f, 0.0f, 1.0f, 0.0f,
				1.0f, 0.0f, 1.0f, 0.0f,
				2.0f / 3.0f, 0.0f, 4.0f / 3.0f, -1.0f / 3.0f };
			
			cudaMemcpy(Coeficient, Coeficient_CPU, 3*4*sizeof(float), cudaMemcpyHostToDevice);
		...
		std::ostream & PrintInfo(std::ostream & out){
			out << "\t\tIntegration Method Type: " << Bifixed_BDFn_GPU_C_INTERFACE::GetName() << endl;
			out << "\t\tBDF order: " << this->BDForder << endl;
			out << "\t\tIntegration Step Time: " << this->elapsedTimeInSeconds<<"s" << endl;
			out << "\t\tNumber of Bifixed Steps: " << this->N_BifixedSteps << endl;
		...
		virtual std::map<std::string, boost::any> GetParameters() const{
			std::map<std::string, boost::any> newMap = BifixedStep_GPU_C_INTERFACE<Neuron_Model_GPU>::GetParameters();
			newMap["bdf_order"] = this->BDForder;
			newMap["name"] = Bifixed_BDFn_GPU_C_INTERFACE::GetName();
			return newMap;
		}
		...
		virtual void SetParameters(std::map<std::string, boost::any> param_map) throw (EDLUTException){
			...
			BifixedStep_GPU_C_INTERFACE<Neuron_Model_GPU>::SetParameters(param_map);
		}
		...
		static std::map<std::string, boost::any> GetDefaultParameters(){
			std::map<std::string, boost::any> newMap = BifixedStep_GPU_C_INTERFACE<Neuron_Model_GPU>::GetDefaultParameters();
			newMap["name"] = Bifixed_BDFn_GPU_C_INTERFACE::GetName();
			newMap["bdf_order"] = 2;
			return newMap;
		}
...
//REVISAR ESTO Y LA VERSION CPU//			
				throw EDLUTException(TASK_BDF_ORDER_LOAD, ERROR_BDF_ORDER_VALUE, REPAIR_BDF_ORDER);
			}

			//load integration time step.
			ModelDescription nmodel = BifixedStep_GPU_C_INTERFACE<Neuron_Model_GPU>::ParseIntegrationMethod(fh, Currentline);

			nmodel.param_map["bdf_order"] = BDF_Order;
			nmodel.model_name = Bifixed_BDFn_GPU_C_INTERFACE::GetName();
			return nmodel;
		}
		...
		*/
		static IntegrationMethod_GPU_C_INTERFACE* CreateIntegrationMethod(ModelDescription nmDescription, Neuron_Model_GPU *nmodel){
			Bifixed_BDFn_GPU_C_INTERFACE * newmodel = new Bifixed_BDFn_GPU_C_INTERFACE(nmodel);
			...
			if (!BifixedStep_GPU_C_INTERFACE<Neuron_Model_GPU>::compare(rhs)){
				return false;
			}
			const Bifixed_BDFn_GPU_C_INTERFACE *e = dynamic_cast<const Bifixed_BDFn_GPU_C_INTERFACE *> (rhs);
			if (e == 0) return false;
