 ...
 * \date May 2013
 *
 * This file declares a class which implements six BDF (Backward Differentiation Formulas) integration methods (from
...
class BDFn : public FixedStep<Neuron_Model> {
    private:
		 ...
		 */
		BDFn():FixedStep<Neuron_Model>(), BDForder(1), PreviousNeuronState(0), D(0), state(0){
		};
	...
	public:
		...
		*/
		 ...
		 */
		BDFn(Neuron_Model * NewModel) : FixedStep<Neuron_Model>(NewModel), BDForder(1), PreviousNeuronState(0), D(0), state(0){
		};
		...
		void NextDifferentialEquationValues(int index, float * NeuronState){
			...
			while (epsi>1e-6 && k<10){
				this->neuron_model->EvaluateDifferentialEquation(AuxNeuronState_p, AuxNeuronState, index, this->elapsedTimeInNeuronModelScale);

				for (int j = 0; j<this->neuron_model->N_DifferentialNeuronState; j++){
					AuxNeuronState_c[j] = BDF_Coeficients[state[index]][0] * this->elapsedTimeInNeuronModelScale*AuxNeuronState[j] + BDF_Coeficients[state[index]][1] * NeuronState[j];
				...
				for (int z = 0; z<this->neuron_model->N_DifferentialNeuronState; z++){
					for (int t = 0; t<this->neuron_model->N_DifferentialNeuronState; t++){
						J[z*this->neuron_model->N_DifferentialNeuronState + t] = BDF_Coeficients[state[index]][0] * this->elapsedTimeInNeuronModelScale * jacnum[z*this->neuron_model->N_DifferentialNeuronState + t];
						if (z == t){
		...
		*/
		...
		virtual ostream & PrintInfo(ostream & out){
			out << "\t\tIntegration Method Type: " << BDFn::GetName() << endl;
	   ...
	   */
		...
		static ModelDescription ParseIntegrationMethod(FILE * fh, long & Currentline) throw (EDLUTException){
		    ...
		    nmodel.model_name = BDFn::GetName();
			return nmodel;
		};
        ...
        virtual bool compare(const IntegrationMethod * rhs) const{
            ...
            const BDFn * e = dynamic_cast<const BDFn *> (rhs);
            if (e == 0) return false;
