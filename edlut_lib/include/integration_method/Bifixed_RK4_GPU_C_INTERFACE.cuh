/***************************************************************************
 *                           Bifixed_RK4_GPU_C_INTERFACE.cuh               *
 *                           -------------------                           *
...
#ifndef BIFIXED_RK4_GPU_C_INTERFACE_H_
#define BIFIXED_RK4_GPU_C_INTERFACE_H_

/*!
 * \file Bifixed_RK4_GPU_C_INTERFACE.cuh
 *
...
/*!
 * \class Bifixed_RK4_GPU_C_INTERFACE
 *
...
class Bifixed_RK4_GPU_C_INTERFACE: public BifixedStep_GPU_C_INTERFACE<Neuron_Model_GPU>{
	public:
     ...
     */
		Bifixed_RK4_GPU_C_INTERFACE():BifixedStep_GPU_C_INTERFACE<Neuron_Model_GPU>(), AuxNeuronState(0), AuxNeuronState1(0), AuxNeuronState2(0), AuxNeuronState3(0), AuxNeuronState4(0){
		}
		 ...
		 *
		 * It generates a new Bifixed RK4 object.
		 *
		* \param NewModel pointer to a template class representing the neuron model.
		*/

		Bifixed_RK4_GPU_C_INTERFACE(Neuron_Model_GPU *  NewModel):BifixedStep_GPU_C_INTERFACE<Neuron_Model_GPU>(NewModel), AuxNeuronState(0), AuxNeuronState1(0), AuxNeuronState2(0), AuxNeuronState3(0), AuxNeuronState4(0){
		}
		 ...
		 */
		virtual ~Bifixed_RK4_GPU_C_INTERFACE(){
			if (AuxNeuronState != 0){
		...
		std::ostream & PrintInfo(std::ostream & out){
			out << "\t\tIntegration Method Type: " << Bifixed_RK4_GPU_C_INTERFACE::GetName() << endl;
		...
		virtual std::map<std::string, boost::any> GetParameters() const{
			...
			newMap["name"] = Bifixed_RK4_GPU_C_INTERFACE::GetName();
			return newMap;
		}
		...
		static ModelDescription ParseIntegrationMethod(FILE * fh, long & Currentline) throw (EDLUTException){
			...
			nmodel.model_name = Bifixed_RK4_GPU_C_INTERFACE::GetName();
			return nmodel;
		}
		...
		*/
		static IntegrationMethod_GPU_C_INTERFACE* CreateIntegrationMethod(ModelDescription nmDescription, Neuron_Model_GPU *nmodel){
			Bifixed_RK4_GPU_C_INTERFACE * newmodel = new Bifixed_RK4_GPU_C_INTERFACE(nmodel);
			...
			const Bifixed_RK4_GPU_C_INTERFACE *e = dynamic_cast<const Bifixed_RK4_GPU_C_INTERFACE *> (rhs);
			if (e == 0) return false;
			return true;
	        };
...
#endif /* Bifixed_RK4_GPU_C_INTERFACE_H_ */
