 ...
 * \date June 2015
 *
 * This file declares a class which implements an adaptive fourth order Runge Kutta integration method. This class implement a multi step
 * integration method.
 */
...
class Bifixed_RK4 : public BifixedStep<Neuron_Model> {
    ...
    private:
         ...
         */
		Bifixed_RK4():BifixedStep<Neuron_Model>(){
		};
    ...
    public:
		 ...
		 */
		Bifixed_RK4(Neuron_Model * NewModel) : BifixedStep<Neuron_Model>(NewModel){
		};
		...
		void NextDifferentialEquationValues(int index, float * NeuronState){
			...
			if (this->integrationMethodState[index]>0){
				for (int iteration = 0; iteration<this->ratioLargerSmallerSteps; iteration++){
					...
					const float BifixedElapsedTimeInNeuronModelScale_0_5 = this->BifixedElapsedTimeInNeuronModelScale*0.5f;
					const float BifixedElapsedTimeInNeuronModelScale_0_16 = this->BifixedElapsedTimeInNeuronModelScale*0.166666666667f;
					...
					this->neuron_model->EvaluateDifferentialEquation(AuxNeuronState, AuxNeuronState2, index, this->BifixedElapsedTimeInNeuronModelScale);
					...
					this->neuron_model->EvaluateDifferentialEquation(AuxNeuronState, AuxNeuronState3, index, this->BifixedElapsedTimeInNeuronModelScale);

					//4rd term
					for (int j = 0; j<this->neuron_model->N_DifferentialNeuronState; j++){
						AuxNeuronState[j] = NeuronState[j] + AuxNeuronState3[j] * this->BifixedElapsedTimeInNeuronModelScale;
					}

					this->neuron_model->EvaluateTimeDependentEquation(AuxNeuronState, index, 1);
					this->neuron_model->EvaluateDifferentialEquation(AuxNeuronState, AuxNeuronState4, index, this->BifixedElapsedTimeInNeuronModelScale);
		...
		*/
		...
		virtual ostream & PrintInfo(ostream & out){
			out << "\t\tIntegration Method Type: " << Bifixed_RK4::GetName() << endl;
      ...
      out << "\t\tNumber of Bifixed Steps: " << this->ratioLargerSmallerSteps << endl;
		 ...
		 */
		 void Calculate_conductance_exp_values(){
			...
			//index 1
			this->neuron_model->Calculate_conductance_exp_values(1, this->BifixedElapsedTimeInNeuronModelScale*0.5f);
		};
	   ...
	   */
		virtual std::map<std::string,boost::any> GetParameters() const{
			...
			newMap["name"] = Bifixed_RK4::GetName();
			return newMap;
		};
		...
		static ModelDescription ParseIntegrationMethod(FILE * fh, long & Currentline) throw (EDLUTException){
		    ...
		    nmodel.model_name = Bifixed_RK4::GetName();
			return nmodel;
		};
		...
		static IntegrationMethod* CreateIntegrationMethod(ModelDescription nmDescription, Neuron_Model *nmodel){
		    Bifixed_RK4 * newmodel = new Bifixed_RK4(nmodel);
        ...
        virtual bool compare(const IntegrationMethod * rhs) const{
            ...
            const Bifixed_RK4 * e = dynamic_cast<const Bifixed_RK4 *> (rhs);
            if (e == 0) return false;
            return true;
        };
};
